...
class emp_page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.archiveBtn = tk.Button(
            self.view_frame, text='Archive Employee', command=lambda: self.archive_emp())
        
    def archive_emp(self):
        if self.controller.frames['admin_page'].tableState == 'active':
            res = askyesno("Archive Employee", "Are you sure you want to archive employee?")
            if res:
                EmpDat.archive_employee(self.employee_selected.id)
                self.controller.frames['admin_page'].emp_tree_init(EmpDat.emp_list)
                self.controller.show_frame("admin_page")
            else:
                return
        elif self.controller.frames['admin_page'].tableState == 'archived':
            res = askyesno("Activate Employee", "Are you sure you want to re-activate employee?")
            if res:
                EmpDat.unarchive_employee(self.employee_selected.id)
                self.controller.frames['admin_page'].emp_tree_init(EmpDat.archived_list)
                self.controller.show_frame("admin_page")
            else:
                return
...
class admin_page(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.switchTableBtn = tk.Button(
            self, text="Switch To Archived Employees", command=lambda: self.switchTable())
